...
- Submodule boundaries are strict:
  - [modules/workspace/main.tf](../modules/workspace/main.tf): `tfe_project`, `tfe_workspace`, workspace vars, team access, run triggers, Sentinel attachments.
  - [modules/vault-auth/main.tf](../modules/vault-auth/main.tf): JWT backend, Vault policies, JWT auth roles.
  - [modules/vault-namespace/main.tf](../modules/vault-namespace/main.tf): optional namespace + KV v2 mount.
...
- Plan/apply separation is implemented end-to-end: role claims in [locals.tf](../locals.tf), role resources in [modules/vault-auth/main.tf](../modules/vault-auth/main.tf), and workspace env vars in [modules/workspace/main.tf](../modules/workspace/main.tf).
...
- Prefer map-driven expansion (`for_each`, `setproduct`) over repeated resources (pattern in [modules/workspace/main.tf](../modules/workspace/main.tf)).
...
- JWT/OIDC trust chain must align: `tfc_hostname` → issuer/discovery URL → Vault JWT backend (`oidc_discovery_url`, `bound_issuer`) in [modules/vault-auth/main.tf](../modules/vault-auth/main.tf).
...
- Workspace Vault env vars (`TFC_VAULT_*`) in [modules/workspace/main.tf](../modules/workspace/main.tf) must match created Vault roles and auth path.
